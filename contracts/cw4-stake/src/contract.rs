...
use crate::state::{Config, ADMIN, CLAIMS, CONFIG, MEMBERS, STAKE, TOTAL};
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::Bond {} => execute_bond(deps, env, NativeBalance(info.funds), info.sender),
...
pub fn execute_bond(
    ...
    let amount = must_pay_funds(&have, &cfg.denom)?;
...
pub fn execute_claim(
    ...
    let amount_str = coin_to_string(release, config.denom.as_str());
